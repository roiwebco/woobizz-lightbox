...
* Admin WooBizz List
*/
if ( ! class_exists('WoobizzLightBoxOptions') ) {
	class WoobizzLightBoxOptions{
		...
		public function __construct( 
			...
			$this->_prefix	= $prefix;
			$this->_url		= ( $url ) ? $url : plugins_url().'/woobizz-lightbox';
			$this->_folder	= $folder;
			...
			if ( $this->_options ) {
				...
				if ( isset( $tabs ) ) {
					...
					foreach ( $tabs as $tab ) {
						...
						if ( isset( ${'tab_'.$tab['id']} ) ) {
							foreach ( ${'tab_'.$tab['id']} as $tab_content ) {
								$option = new WoobizzLightBoxOptionsGenerator( $tab_content, $this->_prefix );
								$div_contents .= $option;
							}
						} else {
					...
					foreach ( ${'tab_0'} as $tab_content ) {
						$option = new WoobizzLightBoxOptionsGenerator( $tab_content );
						$div_contents .= $option;
					}
...
if ( ! class_exists( 'WoobizzLightBoxOptionsGenerator' ) ) {
	class WoobizzLightBoxOptionsGenerator {
